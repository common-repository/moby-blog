...
class RES_JSON_API_Query {
	...
	function get_legacy_controller($json) {
		...
		if ($json == 'submit_comment') {
			if ($res_json_api->controller_is_active('respond')) {
				return 'respond';
			} else {
				$res_json_api->error("The 'submit_comment' method has been removed from the Core controller. To use this method you must enable the Respond controller from WP Admin > Settings > JSON API.");
			}
		} else if ($json == 'create_post') {
			if ($res_json_api->controller_is_active('posts')) {
				return 'posts';
			} else {
				$res_json_api->error("The 'create_post' method has been removed from the Core controller. To use this method you must enable the Posts controller from WP Admin > Settings > JSON API.");
			}
		} else {
			return 'core';
		}
	...
	function get_method($controller) {
		...
		if (empty($method)) {
			...
			return false;
		} else if (method_exists("RES_JSON_API_{$controller}_Controller", $method)) {
			...
			if (is_search()) {
				...
				if (empty($_GET['res-json'])) {
					$res_json_api->error("Uknown method '$method'.");
				}
