...
class RES_JSON_API_Response {

	function setup() {
		global $res_json_api;
		$this->include_values = array();
		$this->exclude_values = array();
		if ($res_json_api->query->include) {
			$this->include_values = explode(',', $res_json_api->query->include);
		}
		// Props to ikesyo for submitting a fix!
		if ($res_json_api->query->exclude) {
			$this->exclude_values = explode(',', $res_json_api->query->exclude);
	...
	function get_json($data, $status = 'ok') {
		...
		$data = apply_filters('res_json_api_encode', $data);
		...
		if (function_exists('json_encode')) {
			...
			if (version_compare(PHP_VERSION, '5.3') < 0) {
				...
				if ($res_json_api->query->json_encode_options) {
					$json_encode_options = $res_json_api->query->json_encode_options;
				}
		...
		if ($res_json_api->query->json_unescaped_unicode) {
	...
	function respond($result, $status = 'ok') {
		...
		if ($res_json_api->query->dev || !empty($_REQUEST['dev'])) {
		...
		} else if ($res_json_api->query->redirect) {
			$url = $this->add_status_query_var($res_json_api->query->redirect, $status);
			wp_redirect($url);
		} else if ($res_json_api->query->callback) {
			// Run a JSONP-style callback with the result
			$this->callback($res_json_api->query->callback, $json);
		} else {
			// Output the result
			$this->output($json);
		}
