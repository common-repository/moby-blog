...
class RES_JSON_API_Author {
	...
	// Note:
	//   RES_JSON_API_Author objects can include additional values by using the
	//   author_meta query var.

	function RES_JSON_API_Author($id = null) {
		if ($id) {
			$this->id = (int) $id;
		} else {
	...
	function set_author_meta() {
		global $res_json_api;
		if (!$res_json_api->query->author_meta) {
			return;
		}
		...
		$vars = explode(',', $res_json_api->query->author_meta);
